...
const osintSearchSchema = Joi.object({
  firstName: Joi.string().max(64).trim().optional(),
  lastName: Joi.string().max(64).trim().optional(),
  filters: Joi.object({
    city: Joi.string().max(80).allow('', null),
    province: Joi.string().max(80).allow('', null),
    ...
    timeRange: Joi.string().max(32).allow('', null)
  }).optional(),
  aliases: Joi.array().items(Joi.string().max(32)).max(5).optional(),
  target: Joi.string().max(255).trim().optional(),
  type: Joi.string().valid('domain', 'person', 'company', 'general', 'email').optional()
}).or('firstName', 'target').unknown(false);
