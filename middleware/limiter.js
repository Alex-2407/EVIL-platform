...
const virtualLabLimiter = (req, res, next) => {
  const key = `vlab:${req.user?.id || req.ip}`;
  const maxAttempts = parseInt(process.env.RATE_LIMIT_VLAB_MAX || 300);
  const windowMs = parseInt(process.env.RATE_LIMIT_VLAB_WINDOW_MS || 3600000);
