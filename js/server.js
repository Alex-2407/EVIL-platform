...
app.get('/api/virtual-lab/catalog', incidentsPublicLimiter, (req, res) => {
  res.json({
...
app.post('/api/virtual-lab/sessions', incidentsPublicLimiter, (req, res) => {
  try {
    const labId = sanitizeString(req.body?.labId || '');
    if (!labId) {
      return res.status(400).json({ error: 'labId obbligatorio' });
    }
    const ownerKey = req.user?.id
      ? `user:${req.user.id}`
      : `ip:${req.ip || 'unknown'}`;
    const session = virtualLabService.createSession(labId, ownerKey);
...
app.get('/api/virtual-lab/sessions/:id', incidentsPublicLimiter, (req, res) => {
  try {
    const ownerKey = req.user?.id
      ? `user:${req.user.id}`
      : `ip:${req.ip || 'unknown'}`;
    const session = virtualLabService.getSession(req.params.id, ownerKey);
...
app.post('/api/virtual-lab/sessions/:id/exec', virtualLabLimiter, (req, res) => {
  try {
    const command = sanitizeString(req.body?.command || '');
    if (!command) {
    ...
    const ownerKey = req.user?.id
      ? `user:${req.user.id}`
      : `ip:${req.ip || 'unknown'}`;
    const result = virtualLabService.execCommand(req.params.id, ownerKey, command);
    res.json(result);
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
  }
...
app.delete('/api/virtual-lab/sessions/:id', incidentsPublicLimiter, (req, res) => {
  try {
    const ownerKey = req.user?.id
      ? `user:${req.user.id}`
      : `ip:${req.ip || 'unknown'}`;
    virtualLabService.stopSession(req.params.id, ownerKey);
    res.json({ ok: true });
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
  }
