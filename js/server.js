...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    if (isDev) {
      ...
      const allowedOrigins = (process.env.CORS_ORIGINS || 'https://example.com').split(',').map(o => o.trim());
...
// ========================
app.post('/api/dns-enum', async (req, res) => {
...
app.post('/api/subdomain-finder', async (req, res) => {
  const { domain } = req.body;
  if (!domain) return res.status(400).json({ error: 'Domain required' });
...
// ========================
app.post('/api/ssl-analyzer', async (req, res) => {
  const { domain } = req.body;
  if (!domain) return res.status(400).json({ error: 'Domain required' });
...
app.get('/api/realtime-incidents', async (req, res) => {
  try {
    let incidents = [];
...
app.post('/api/progress/save', (req, res) => {
  ...
  try {
    ...
    // Salva i progressi nel profilo utente
...
app.get('/api/progress/load', (req, res) => {
  ...
  try {
    ...
    // Ritorna i progressi salvati (o un default se non esiste)
...
app.post('/api/progress/unlock-achievement', (req, res) => {
  const token = req.headers.authorization?.split(' ')[1];
  if (!token) {
    return res.status(401).json({ error: 'Token mancante' });
  }
  ...
  try {
    ...
    res.json({
      ...
      user: {
        ...
        loginHistory: (user.loginHistory || []).slice(-5) // Ultimi 5 login
      }
    });
  } catch (err) {
