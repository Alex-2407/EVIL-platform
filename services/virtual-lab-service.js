...
function createSession(labId, ownerKey) {
  ...
  const ownerSessions = [...sessions.values()].filter(
    (s) => s.ownerKey === ownerKey && s.status === 'running'
  );
...
function getSession(sessionId, ownerKey) {
...
function stopSession(sessionId, ownerKey) {
...
function execCommand(sessionId, ownerKey, commandLine) {
  const session = getSession(sessionId, ownerKey);
